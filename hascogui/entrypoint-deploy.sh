...
dedupe_bootstrap_barrio_schema() {
    ...
    if [ -f "$schema" ]; then
        ...
        tmp=$(mktemp)
        awk '
            /^bootstrap_barrio_messages_widget_toast_delay:/ {inside=1; seen_type=0}
            inside && /^[^[:space:]]/ && !/^bootstrap_barrio_messages_widget_toast_delay:/ {inside=0}
            { if (inside && $1=="type:") { if (seen_type) next; seen_type=1 } }
            {print}
